...
*New * - This addon now includes a "handwritten" rendering path where the goal is to be able to perform the Perspective / View / Model Matrix transforms on a series of points in an easy enough way that it can be done with only a calculator in order to get accurate linework before the rest of the artwork is done.
